...
export default function MatchdayPage() {
  ...
  const gettingHere = useMemo(() => [
    ...
    { icon: '🚌', title: 'By Bus', text: settings.byBusInformation || 'The number 67 bus runs from Stroud town centre to Brimscombe. Alight at the War Memorial stop on London Road, approximately a two-minute walk from the ground. Journey time from Stroud is around seven minutes, with services normally running every 30 minutes.', link: { label: 'View 67 Bus Timetable', url: settings.busTimetableUrl || FALLBACK_BUS_URL } },
    { icon: '🚆', title: 'By Train', text: settings.byTrainInformation || 'The nearest railway station is Stroud, served by GWR with regular services from Gloucester, Swindon and London Paddington. From Stroud, take the number 67 bus or a taxi to the ground.', link: null },
    ...
    <main style={{ background: '#F2F2F2', minHeight: '100vh', padding: '0 0 90px' }}>
      <section style={{ maxWidth: 980, margin: '0 auto', padding: '52px 24px' }}>
        ...
        <div style={{ marginBottom: 44 }}>
          ...
          <a href={what3wordsUrl} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginBottom: 24, padding: '9px 14px', borderRadius: 6, background: '#fff', border: '1px solid #D1D5DB', color: '#041B5F', fontFamily: "'Montserrat', sans-serif", fontSize: 12, fontWeight: 800, textDecoration: 'none' }}><span style={{ color: '#E11D48', fontSize: 16 }}>///</span>{what3words}</a>
          <div style={{ borderRadius: 8, overflow: 'hidden', marginBottom: 20, border: '1px solid #E5E7EB' }}><iframe src={settings.googleMapsEmbedUrl || FALLBACK_MAP_EMBED} width="100%" height="280" style={{ border: 0, display: 'block' }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" title={`${groundName} location`} /></div>
          <div className="mobile-card-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 20 }}>
            {gettingHere.map(g => <div key={g.title} style={{ ...card, width: '100%', maxWidth: 360, margin: '0 auto' }}><div style={{ fontSize: 22, marginBottom: 8 }}>{g.icon}</div><h3 style={h3}>{g.title}</h3><p style={{ ...body, marginBottom: g.link ? 10 : 0 }}>{g.text}</p>{g.link && <a href={g.link.url} target="_blank" rel="noopener noreferrer" style={{ fontFamily: "'Montserrat', sans-serif", fontSize: 12, fontWeight: 700, color: '#1149D8', textDecoration: 'none' }}>{g.link.label} →</a>}</div>)}
          </div>
        </div>
