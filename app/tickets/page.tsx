...
const fallbackFaqs = [
  { question: 'Can I pay in instalments?', answer: 'Not currently — season tickets must be paid in full at time of purchase.' },
  { question: 'How do I get my ticket?', answer: 'If you use an iPhone, you can add your digital season ticket to Apple Wallet. If you use Android, or would prefer a PDF, your ticket will be emailed to you.' },
  { question: 'What is included?', answer: 'Your season ticket covers all First XI home league and club cup fixtures throughout the season. FA Vase matches are excluded.' },
  { question: 'Is my payment secure?', answer: 'Yes. Payments are processed securely by Stripe.' },
  { question: 'Do under 16s pay?', answer: 'Under 16s are admitted free to all First XI matches. Proof of age may be requested.' },
  { question: 'What about friendlies?', answer: 'Pre-season and friendly matches are charged separately. Season tickets do not cover friendly fixtures.' },
  { question: 'Cash or card?', answer: 'We accept both cash and card at the gate on matchday.' },
  { question: 'When does the gate open?', answer: 'The turnstiles normally open approximately one hour before kick-off.' },
]
  ...
  return (
    <main style={{ background: '#F2F2F2', minHeight: '100vh', padding: '0 0 90px' }}>
      <section style={{ maxWidth: 980, margin: '0 auto', padding: '52px 24px' }}>
        <div style={{ background: '#041B5F', borderRadius: 8, padding: '32px 36px', color: '#fff', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 32, alignItems: 'center', marginBottom: 28 }}>
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 12, maxWidth: 420 }}>
              {seasonPrices.map(([label, price]) => (
                <div key={label} style={{ background: 'rgba(255,255,255,.08)', border: '1px solid rgba(255,255,255,.15)', borderRadius: 6, padding: '10px 16px', textAlign: 'center', width: '100%', maxWidth: 140, margin: '0 auto' }}>
                  <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: 10, color: 'rgba(255,255,255,.55)', letterSpacing: '.1em', textTransform: 'uppercase' }}>{label}</div>
                  <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 800, fontSize: 26, color: price === 'Free' ? '#22C55E' : '#fff' }}>{price}</div>
                </div>
              ))}
            </div>
        ...
        <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 8, padding: 28, marginBottom: 52 }}>
          <div style={{ height: 4, background: '#1149D8', marginBottom: 20, borderRadius: 2 }} />
          <h2 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 28, fontWeight: 800, color: '#2D2D2D', margin: '0 0 6px', letterSpacing: '0.03em' }}>Buy Your Season Ticket</h2>
          <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: 12, color: '#6B7280', margin: '0 0 20px' }}>Select your ticket type below and complete your purchase.</p>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 320px), 1fr))', gap: 12, marginBottom: 24, padding: 18, background: '#EEF4FF', border: '1px solid #BFDBFE', borderLeft: '5px solid #1149D8', borderRadius: 8 }}>
            <div>
              <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 19, fontWeight: 800, color: '#041B5F', marginBottom: 5 }}>🍎 Using an iPhone?</div>
              <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: 12, color: '#374151', lineHeight: 1.6, margin: 0 }}>You can add your digital season ticket to <strong>Apple Wallet</strong> for quick and easy access at the turnstile.</p>
            </div>
            <div>
              <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 19, fontWeight: 800, color: '#041B5F', marginBottom: 5 }}>📧 Using Android or prefer a PDF?</div>
              <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: 12, color: '#374151', lineHeight: 1.6, margin: 0 }}>We will email your season ticket to you as a <strong>PDF</strong>. Simply open it on your phone and show the QR code at the gate.</p>
            </div>
            <p style={{ gridColumn: '1 / -1', fontFamily: "'Montserrat', sans-serif", fontSize: 11, color: '#6B7280', lineHeight: 1.5, margin: 0 }}>Both formats contain the same QR code and are equally valid for matchday entry.</p>
          </div>
        ...
        <div style={{ marginBottom: 52 }}>
          ...
          <div className="mobile-card-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 20, marginBottom: 20 }}>
            <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 8, padding: 24, width: '100%', maxWidth: 420, margin: '0 auto' }}>
              ...
              <h3 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 22, fontWeight: 800, color: '#2D2D2D', margin: '0 0 16px' }}>League &amp; Cup <span style={{ fontSize: 15, color: '#6B7280' }}>(except FA Vase)</span></h3>
              {admissionPrices.map(row => (
                <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid #F3F4F6' }}>
                  <span style={{ fontFamily: "'Montserrat', sans-serif", fontSize: 13, color: '#374151' }}>{row.label}</span>
        ...
        <div>
          <h2 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 36, fontWeight: 800, color: '#2D2D2D', margin: '0 0 20px', letterSpacing: '0.03em' }}>FAQs</h2>
          <div className="mobile-card-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 20 }}>
            {faqs.map((faq: any) => (
              <div className="mobile-full-card" key={faq.question} style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 8, padding: 22, minHeight: 150, width: '100%', maxWidth: 360, margin: '0 auto' }}>
                <h3 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 22, fontWeight: 800, color: '#2D2D2D', margin: '0 0 12px', lineHeight: 1.1 }}>{faq.question}</h3>
                <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: 12, color: '#4B5563', lineHeight: 1.65, margin: 0 }}>{faq.answer}</p>
              </div>
            ))}
          </div>
        </div>
