...
function MobileCard({ fixture }: { fixture: Fixture }) {
  ...
  return <article style={{ background: '#fff', border: '1px solid #DCE3F1', borderLeft: '5px solid #1149D8', borderRadius: 8, padding: 16, boxShadow: '0 5px 16px rgba(4,27,95,.06)' }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}><div><div style={{ color: '#6B7280', fontFamily: "'Montserrat',sans-serif", fontSize: 10, fontWeight: 700, textTransform: 'uppercase' }}>{info.day} · {fixture.kickoff || 'TBC'} · {fixture.venue}</div><h3 style={{ margin: '6px 0 3px', color: '#041B5F', fontFamily: "'Barlow Condensed',sans-serif", fontSize: 22, fontWeight: 800 }}>{fixture.opponent}</h3><div style={{ color: '#6B7280', fontFamily: "'Montserrat',sans-serif", fontSize: 11 }}>{fixture.competition || 'Competition TBC'}</div></div><div style={{ flexShrink: 0, textAlign: 'center' }}><div style={{ color: '#041B5F', fontFamily: "'Barlow Condensed',sans-serif", fontSize: 25, fontWeight: 800 }}>{resultFor(fixture)}</div>{form && <span style={{ display: 'inline-flex', width: 28, height: 28, marginTop: 7, alignItems: 'center', justifyContent: 'center', background: colour, color: '#fff', borderRadius: 4, fontWeight: 800 }}>{form}</span>}</div></div>{((fixture.played && fixture.sourceUrl) || penalty) && <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 13, flexWrap: 'wrap' }}>{fixture.played && fixture.sourceUrl && <a href={fixture.sourceUrl} target="_blank" rel="noopener noreferrer" style={{ color: '#1149D8', fontFamily: "'Montserrat',sans-serif", fontSize: 11, fontWeight: 800, textDecoration: 'none', textTransform: 'uppercase' }}>Match Details →</a>}{penalty && <span style={{ color: penaltyColour, fontFamily: "'Montserrat',sans-serif", fontSize: 10, fontWeight: 800, lineHeight: 1.2, whiteSpace: 'nowrap' }}>{penalty}</span>}</div>}</article>
}
